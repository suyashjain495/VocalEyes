package com.example.vocaleyesnew.facenet_android.domain.face_detection
...
class FaceSpoofDetector(context: Context, useGpu: Boolean = false, useXNNPack: Boolean = false, useNNAPI: Boolean = false) {
    ...
    // Flag to indicate if the models loaded successfully
    private val modelsAvailable: Boolean

    init {
        // Initialize TFLiteInterpreter with fallback
        modelsAvailable = try {
            val interpreterOptions =
                Interpreter.Options().apply {
                    // Due to compatibility issues with GPU delegate, we'll use CPU instead for stability
            ...
            Log.e("FaceSpoofDetector", "Error initializing models: ${e.message}")
            false
        }
    }
    ...
    suspend fun detectSpoof(frameImage: Bitmap, faceRect: Rect): FaceSpoofResult =
        withContext(Dispatchers.Default) {
            // If models aren't available, return a safe default (not a spoof)
            if (!modelsAvailable || firstModelInterpreter == null || secondModelInterpreter == null) {
                Log.w("FaceSpoofDetector", "Models not available, returning default result")
                return@withContext FaceSpoofResult(isSpoof = false, score = 1.0f, timeMillis = 0)
            }
            ...
            try {
                ...
                Log.e("FaceSpoofDetector", "Error during detection: ${e.message}")
                // Return safe default in case of any runtime error
                return@withContext FaceSpoofResult(isSpoof = false, score = 1.0f, timeMillis = 0)
            }
        }
